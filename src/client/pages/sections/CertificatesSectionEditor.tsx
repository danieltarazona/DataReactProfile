...
export function CertificatesSectionEditor({ data, activeRoleId, isVisibleForRole, t, lang, sectHook }: SectionEditorProps) {
    const renderLocalizedFields = (item: any, baseField: string, label: string, isTextarea: boolean = false) => (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-l-2 border-orange-500/20 pl-4 py-1">
    ...
    return (
        <div className="space-y-12 pb-24 animate-in fade-in duration-500">
            <div className="flex items-center justify-between mb-8">
                ...
                <Button onClick={() => sectHook.addEntry({ nameEn: 'New Certificate' })} variant="solid" size="sm">
                            ...
                            <div className="flex-1 space-y-6">
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] text-orange-400 font-bold uppercase tracking-widest px-1">Certificate Name</label>
                                        {renderLocalizedFields(item, 'name', 'Name')}
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-[10px] text-orange-400 font-bold uppercase tracking-widest px-1">Issuer</label>
                                        {renderLocalizedFields(item, 'issuer', 'Issuer')}
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-[10px] text-orange-400 font-bold uppercase tracking-widest px-1">Location</label>
                                        {renderLocalizedFields(item, 'location', 'Location')}
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-[10px] text-orange-400 font-bold uppercase tracking-widest px-1">Description</label>
